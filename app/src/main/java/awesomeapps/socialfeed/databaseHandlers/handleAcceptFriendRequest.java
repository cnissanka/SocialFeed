 ...
 */
public class handleAcceptFriendRequest extends AsyncTask<String,Void,String>{
    private Context context;

    public handleAcceptFriendRequest(Context ctx){
        this.context = ctx;
    }

    @Override
    protected String doInBackground(String... params) {
        String register_url = "http://192.168.43.59:8080/SocialFeed/handleAcceptFriendRequest";
        try {
    ...
    protected void onPostExecute(String s) {
        Log.i("messagefromtheotherside",s);
        if (s.equals("success")){
            AlertDialog alert = new AlertDialog.Builder(context).create();
            alert.setMessage("Accepted Successfully : You are now Friends!");
            alert.setTitle("Friend Msg");
            alert.show();
        }
    }
